                   ...
                   const uint16_t stride, const uint16_t dim_im_out, q7_t * bufferA, q7_t * Im_out)
{
    ...
    for (i_y = 0; i_y < dim_im_in; i_y++, row0 += dim_im_in)
    {
		int32_t ofs = row0;
		int32_t rowOfs = 0 - padding;
        for (i_x = 0; i_x < dim_im_out; i_x++ , rowOfs += stride, ofs += stride)
        {
            /* for each output pixel */
            q7_t     *target = Im_in + (row0 + i_x) * ch_im_in;
            q7_t     *win_start;
            q7_t     *win_stop;
            ...
            {
                win_start = Im_in + ofs * ch_im_in;
            }

            if (rowOfs + dim_kernel >= dim_im_in)
            {
                win_stop = Im_in + (row0 + dim_im_in) * ch_im_in;
            } else
            {
                win_stop = Im_in + (ofs + dim_kernel) * ch_im_in;
            }
	...
	int32_t rowStride = dim_im_in * ch_im_in;
	
    for (i_y = 0; i_y < dim_im_out; i_y++)
    {

        /* for each output row */
        q7_t     *target = Im_out + i_y * rowStride;
        q7_t     *row_start;
        q7_t     *row_end;
        ...
        {
            row_start = Im_in + (i_y * stride - padding) * rowStride;
        }
        /* setting the stopping row */
        if (i_y * stride - padding + dim_kernel >= dim_im_in)
        {
            row_end = Im_in + dim_im_in * rowStride;
        } else
        {
            row_end = Im_in + (i_y * stride - padding + dim_kernel) * rowStride;
        }

        /* copy over the first row */
        /* arm_copy_q7(row_start, target, dim_im_out * ch_im_in); */
        memmove(target, row_start, rowStride);

        /* move over to next row */
        row_start += ch_im_in * dim_im_in;

        for (; row_start < row_end; row_start += rowStride)
        {
            compare_and_replace_if_larger_q7(target, row_start, rowStride);
        }
    }

PINS_AF = (
  ('LED_R', (2, 'LPUART8_RX'), (3, 'SAI3_TX_SYNC'), (5, 'GPIO3_PIN25'), ),
  ('LED_G', (1, 'GPT2_CAPTURE2'), (5, 'GPIO3_PIN26'), ),
  ('LED_B', (1, 'GPT2_CAPTURE1'), (5, 'GPIO3_PIN27'), ),
  ('LED_IR', (5, 'GPIO2_PIN31'), ),
  ('KEY', (5, 'GPIO5_PIN0'), ),
  ('DBG_RXD', (0, 'LPI2C4_SDA'), (1, 'GPT1_CLK'), (2, 'LPUART1_RX'), (4, 'FLEXPWM1_PWMX3'), (5, 'GPIO1_PIN13'), ),
  ('DBG_TXD', (0, 'LPI2C4_SCL'), (2, 'LPUART1_TX'), (5, 'GPIO1_PIN12'), ),
  ('UART4_TXD', (2, 'LPUART4_TX'), (4, 'TMR2_TIMER0'), (5, 'GPIO4_PIN19'), ),
  ('UART4_RXD', (2, 'LPUART4_RX'), (4, 'TMR2_TIMER1'), (5, 'GPIO4_PIN20'), ),
  ('SDO', (2, 'LPUART5_CTS_B'), (3, 'LPSPI1_SDO'), (5, 'GPIO4_PIN28'), ),
  ('SDI', (2, 'LPUART6_RTS_B'), (3, 'LPSPI1_SDI'), (5, 'GPIO4_PIN29'), ),
  ('SCK', (2, 'LPUART5_RTS_B'), (3, 'LPSPI1_SCK'), (5, 'GPIO4_PIN27'), ),
  ('PCSO', (2, 'LPUART6_CTS_B'), (3, 'LPSPI1_PCS0'), (5, 'GPIO4_PIN30'), ),
  ('SDA3', (2, 'LPI2C3_SDA'), (4, 'TMR2_TIMER2'), (5, 'GPIO4_PIN21'), ),
  ('SCL3', (2, 'LPI2C3_SCL'), (4, 'TMR2_TIMER3'), (5, 'GPIO4_PIN22'), ),
  ('SDA4', (2, 'LPI2C4_SDA'), (5, 'GPIO4_PIN11'), ),
  ('SCL4', (2, 'LPI2C4_SCL'), (5, 'GPIO4_PIN12'), ),
  ('TMR', (1, 'TMR2_TIMER3'), (5, 'GPIO2_PIN25'), ),
)
